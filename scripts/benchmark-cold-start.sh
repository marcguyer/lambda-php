...
echo "Memory,BilledMs,InitMs,MemoryUsedMB" > cold_start_results.csv

echo "Testing cold starts at ${MEMORY}MB with ${SAMPLES} samples..."
...
for i in $(seq 1 "$SAMPLES"); do
    ...
    echo "$MEMORY,$billed,$init,$mem" >> cold_start_results.csv
    echo "  Sample $i: ${billed}ms (init: ${init}ms), ${mem}MB"
done

echo "Complete! Results in cold_start_results.csv"