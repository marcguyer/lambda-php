...
# Deploy base runtime (no HTTP overhead) for benchmarking

FUNCTION_NAME="php-lambda-function"
HANDLER_FILE="function-handler-benchmark.php"

echo "Deploying PHP Lambda BASE runtime to AWS..."
...
if aws lambda get-function --function-name "$FUNCTION_NAME" >/dev/null 2>&1; then
    ...
    aws lambda update-function-configuration \
        ...
        --environment "Variables={LAMBDA_HANDLER=$HANDLER_FILE,LAMBDA_RUNTIME_DEBUG=1}" \
        >/dev/null
else
...
echo ""
echo "Test with:"
echo "  aws lambda invoke --function-name $FUNCTION_NAME --payload '{}' --cli-binary-format raw-in-base64-out response.json"
echo "=========================================="
